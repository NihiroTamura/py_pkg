...
LQR_R = np.array([[20]])              # 制御入力の重み（大きいほど入力を抑える）
...
class MathematicalSolver:
    ...
    def calculate_lqr_ff(self, target_params, sys_params, u_best_ff, y0):                                          # 引数(目標モデルのパラメータ[T1, wn], 目標モデルのパラメータ[a2, a1, a0, b0], ベストFF制御入力, 初期偏差)
        ...
        x_sys = np.array([y0, 0.0, 0.0], dtype=float)                                   # システムモデルの初期状態 (可観測正準形)
        ...
        for i in range(len(self.t_eval)):
            if self.t_eval[i] <= self.T:                                                                # FF入力時間内のみ実行
                ...
                dx0 = x_sys[1]
                dx1 = x_sys[2]
                dx2 = b0_sys * u_total - a0_sys * x_sys[0] - a1_sys * x_sys[1] -a2_sys * x_sys[2]
                x_sys[0] += dx0 * dt
                x_sys[1] += dx1 * dt
                x_sys[2] += dx2 * dt
            else:
