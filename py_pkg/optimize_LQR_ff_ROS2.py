...
class MathematicalSolver:
    ...
    # LQR最適制御 + 5次多項式フィット
    # ------------------------------------------------------------------
    def calculate_lqr_ff(self, target_params, sys_params, u_ff, y0):                                          # 引数(目標モデルのパラメータ[T1, wn], 目標モデルのパラメータ[a2, a1, a0, b0], FF制御入力, 初期偏差)
        """
...
class OptimalControlSequencer(Node):
    ...
    def __init__(self, csv_path, T, max_iter, target_mode):                                                                                     # 引数(保存csv情報, FF制御入力時間, 外側ループ最大回数, 目標値の与え方がランダムorプリセット)
        ...
        self.home_pot = list(self.initial_pot)                                                                  # オリジナルの初期姿勢（非常停止・終了用安定位置）を保存
    ...
    def dispatch_optimization_pipeline(self):
        try:
            ...
            for dof_idx in range(24):                                                                                   # 24自由度分のループ
                ...
                new_ff, extrema, u_opt_full, y_tgt = solver.calculate_lqr_ff(tgt_params, sys_params, u_ff, y0)    # 最適入力を計算して、その結果の、FFパラメータ、極値、5次関数のFF制御入力、目標モデルの応答、を格納
