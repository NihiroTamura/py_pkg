#!/usr/bin/env python3
"""
逆ダイナミクス法 + 5次関数FF制御入力 最適化プログラム (ROS2)
...
class MathematicalSolver:
    ...
    def calculate_optimal_ff(self, target_params, sys_params, y0):                                              # 引数（fit_target_model()で求めたT1・ωn, fit_system_model()で求めたa2・a1・a0・b0, 初期位置）
        ...
        def fit_loss(p):
            ...
            penalty_pwm = np.sum(np.maximum(0, u_pred - 255)**2) + np.sum(np.maximum(0, -255 - u_pred)**2)  # -255から255の間に収めるためのペナルティ
            
            # 0 < t < T の間で極値が必ず2つになるようにするペナルティ
            e_val = -(a*self.T**4 + b*self.T**3 + c*self.T**2 + d*self.T)
            dp = 5*a*t_ff**4 + 4*b*t_ff**3 + 3*c*t_ff**2 + 2*d*t_ff + e_val
            sign_changes = np.count_nonzero(np.diff(dp > 0)) # 微分値の符号が反転した回数(極値の数)
            penalty_extrema = abs(sign_changes - 2) * 1e7
            
            return mse + 1e6 * penalty_pwm + penalty_extrema
        
        # 極値が2つになるような初期推測値 (微小な3次関数の形を初期値とする)
        initial_guess = [0.0, 0.0, 0.01, -0.015 * self.T]
        res = scipy.optimize.minimize(fit_loss, initial_guess, method='Nelder-Mead')                    # 5次関数の係数の最適化を開始
...
class OptimalControlSequencer(Node):
    def __init__(self, csv_path, T, max_iter, target_mode):
        ...
        self.max_inner_iter = 30 # 評価関数Jが最小になるまで繰り返す最大回数
